...
import "./Balloon.css";
import SmallBalloon from "../SmallBalloon/SmallBalloon";
...
const Balloon = ({ id, color, isActive, onClick }) => {
  ...
  const smallBalloon = [];
      for (let j = 0; j < 6; j++) {
        smallBalloon.push(
        <div className={`small-balloon-animate--${j}`}>
        <SmallBalloon
          key={j}
          id={j}
  ...
  if(isPopped) {
    return (
      ...
      in={isPopped}
      timeout={250}
      classNames="balloons-moving"
      mountOnEnter
        unmountOnExit
    >{(state) => (
      <div className={`balloons-screen balloons-screen--${state}`}>
    <span key={`smallBalloon+${id}`} className="small-balloon-cell">
      {smallBalloon}
    </span>
    </div>)}
    </CSSTransition>
